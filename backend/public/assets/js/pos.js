...
async function refreshPosFuels(selectedFuelId = null) {
    ...
    try {
        const response = await fetch('/public/staff/ajax/pos_fuels.php', {
            cache: 'no-store'
        });
...
async function refreshSalesHistory() {
    ...
    try {
        const response = await fetch('/public/staff/ajax/pos_sales_history.php', {
            cache: 'no-store'
        });
...
if (posForm) {
    posForm.addEventListener("submit", async function(e) {
        ...
        try {
            const response = await fetch('/public/staff/ajax/process_sale.php', {
                method: 'POST',
                body: formData
            });
